...
4. **If `openspec/config.yaml` exists, OpenSpec is required for this workflow.** Create or continue the change's planning artifacts before writing any code. Resolve the skill to use at run time rather than assuming a name: list the `openspec-*` skills actually present, match one by purpose against the table in [openspec.md](openspec.md) — creating a change for new work, revising or resuming one that already exists — and use the CLI fallback flow when nothing matches. No skill is guaranteed, not even the ones in the `core` preset, because the installed set comes from a per-machine profile. Say which path you took. Only when the repository has no `openspec/config.yaml` do you proceed directly from the issue.
...
8. Implement in small steps following existing code patterns. When OpenSpec is configured, delegate to the resolved skill for implementing a change's tasks (usually `openspec-apply-change`) and keep the tasks file in sync as work completes.
...
11. When OpenSpec is configured, archive the completed change **with spec synchronization** before opening the PR, delegating to the resolved skill for finalizing a change (usually `openspec-archive-change`) or to `openspec archive <name>` when it is absent. The archive must merge the change's delta specs into the main specs, so never pass `--skip-specs` unless the change is tooling or documentation only. Archive only after validation passes, and commit the resulting spec updates as part of the branch.
