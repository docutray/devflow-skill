...
4. **If `openspec/config.yaml` exists, OpenSpec is required for this workflow.** Create or continue the change's planning artifacts before writing any code. Delegate to `openspec-propose` for new work and to `openspec-update-change` to revise an existing change; both ship in the default profile. `openspec-continue-change` resumes a partially planned change but is not installed by default, so use it only after confirming it exists, and otherwise fall back to `openspec-update-change` or the CLI. Follow [openspec.md](openspec.md). Only when the repository has no `openspec/config.yaml` do you proceed directly from the issue.
